...
public class ImageDirectoryModel implements Serializable{
    ...
    private static final String TAG = "ImageDirectoryModel";
    ...
    public void addImage(String path, long date, long id){
        SingleImageModel image = new SingleImageModel(path, false, date, id,path,0,false);
        images.add(image);
    }
    ...
    public void setImage(String path){
        for (SingleImageModel image : images){
            if (image.isThisImage(path)){
                if(image.isPicked){
                    Log.e(TAG, "this image is picked!!!");
                }
                image.isPicked = true;
                break;
            }
    ...
    public void unsetImage(String path){
        for (SingleImageModel image : images){
            if (image.isThisImage(path)){
                if(!image.isPicked){
                    Log.e(TAG, "this image isn't picked!!!");
                }
